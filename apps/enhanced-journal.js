...
export class EnhancedJournal extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  changeTab(tab, group, options = {}) {
    ...
    game.user.setFlag(
      ...
      `pagestate.${this.document.id}.tabId`,
      tab,
    );
  }
  ...
  _initializeApplicationOptions(options) {
    ...
    options.classes.push(
      "themed",
      `theme-${colorScheme.applications || "dark"}`,
    );
  ...
  async _preFirstRender(context, options) {
    ...
    this.tabs = foundry.utils.duplicate(
      game.user.getFlag("monks-enhanced-journal", "tabs") || [
        {
          id: makeid(),
          text: i18n("MonksEnhancedJournal.NewTab"),
          active: true,
          history: [],
        },
      ],
    ...
    this.tabs.active = (findone = true) => {
      ...
      if (findone) {
        if (tab == undefined && this.tabs.length > 0) tab = this.tabs[0];
      }
      return tab;
    };
    this.bookmarks = foundry.utils.duplicate(
      game.user.getFlag("monks-enhanced-journal", "bookmarks") || [],
    );
  ...
  _prepareBodyContext(context, options) {
    ...
    context = foundry.utils.mergeObject(context, {
      ...
      unavailable: game.user.isGM
        ? cfg.collection?.instance?.invalidDocumentIds?.size
        : 0,
    });
    ...
    const unavailable = game.user.isGM
      ? ui.journal.collection.invalidDocumentIds.size
      : 0;
    if (unavailable) {
      ...
      const label = game.i18n.format(locPath, {
        count: unavailable,
        document: docLabel,
      });
  ...
  get isEditable() {
    ...
    if (
      document instanceof JournalEntryPage &&
      !!foundry.utils.getProperty(document, "flags.monks-enhanced-journal.type")
    ) {
  ...
  _replaceHTML(result, content, options) {
    if (
      this.subsheet &&
      this.subsheet.document?.id &&
      !["blank", "folder"].includes(this.document.type) &&
      !(this.document instanceof Actor)
    ) {
      ...
      if (priorElement) {
        ...
        for (let partId of parts) {
          let partState = {};
          this.subsheet._preSyncPartState.call(
            this.subsheet,
            partId,
            priorElement,
            priorElement,
  ...
  async _onRender(context, options) {
    ...
    $(".bookmark-button:not(.add-bookmark)", this.element).click(
      this.activateBookmark.bind(this),
    );
    ...
    $(".back-button, .forward-button", this.element)
      .toggle(game.user.isGM || setting("allow-player"))
      .on("click", this.navigateHistory.bind(this));
  ...
  async renderSubSheet(options = {}) {
    try {
      ...
      if (!currentTab) {
        if (this.tabs.length) currentTab = this.tabs[0];
        else currentTab = this.addTab();
      }
      if (
        !currentTab.entity &&
      ...
      if (
        this.document?.id != currentTab.entity?.id ||
        currentTab.entity instanceof Promise ||
        currentTab.entity?.id == "blank-journal-entry"
      )
        this.document = currentTab.entity;

      //if there's no object then show the default
      if (this.document instanceof Promise) this.document = await this.document;

      if (
        this.document instanceof JournalEntry &&
        this.document.pages.size == 1 &&
        (!!foundry.utils.getProperty(
      ...
      if (force != true) {
        ...
        if (
          testing instanceof JournalEntryPage &&
          !!foundry.utils.getProperty(
            testing,
            "flags.monks-enhanced-journal.type",
          )
        ...
        if (
          ...
          testing &&
          ((!testing.compendium &&
            testing.testUserPermission &&
            !testing.testUserPermission(game.user, "OBSERVER")) ||
            (testing.compendium && !testing.compendium.visible))
        ) {
      ...
      if (
        this.subsheet?.document?.id != this.document?.id ||
        this.subsheet?.type != this.document?.type
      ) {
        // The subsheet has changed, so we need to re-render it
        const cls =
          this.document instanceof JournalEntry
            ? JournalEntrySheet
            : this.document._getSheetClass
              ? this.document._getSheetClass()
              : null;
        if (!cls || this.document instanceof Actor) {
          if (this.document instanceof Actor)
            ui.notifications.warn(
              i18n("MonksEnhancedJournal.msg.CannotOpenActorInEnhancedJournal"),
            );
          this.subsheet = new BlankSheet({
            document: this.document,
            enhancedjournal: this,
            tabs: this.tabs,
          ...
          subsheetOptions = foundry.utils.mergeObject(
            subsheetOptions,
            game.user.getFlag(
              "monks-enhanced-journal",
              `pagestate.${this.document.id}`,
            ) || {},
          );

          this.subsheet = new cls({
            document: this.document,
      ...
      let tabId = game.user.getFlag(
        "monks-enhanced-journal",
        `pagestate.${this.document.id}.tabId`,
      );
      if (tabId) subsheet.tabGroups = { primary: tabId };
      ...
      if (
        ...
        for (let btn of buttons) {
          if ($("> header a." + btn.class, this.element).length == 0) {
            //don't repeat buttons
            a = $("<a>")
              .addClass(btn.class)
              .addClass("subsheet")
              .toggleClass("first", first)
        ...
        if (a) a.addClass("last");
      }

      subsheet.enhancedjournal = this;

      subsheet._configureRenderOptions.call(subsheet, subsheetOptions);
      let subsheetContext = await subsheet._prepareContext.call(
        subsheet,
        subsheetOptions,
      );

      await subsheet._preRender.call(
        subsheet,
        subsheetContext,
        subsheetOptions,
      );
      ...
      let result = await subsheet._renderHTML.call(
        subsheet,
        subsheetContext,
        subsheetOptions,
      );

      $(".window-title", this.element).html(
        (subsheet.title || i18n("MonksEnhancedJournal.NewTab")) +
          " - " +
      ...
      if (game.system.id == "pf2e") classes += " journal-page-content";
      if (!(subsheet instanceof foundry.appv1.sheets.ActorSheet)) {
        if (setting("use-system-tag")) classes += " " + game.system.id;
      }

      if (this.document instanceof JournalEntry) {
        classes +=
          subsheet?.mode === modes.MULTIPLE
            ? " multiple-pages"
            : " single-page";
      }
      ...
      if (
        subsheet &&
        subsheet.document?.id &&
        this.subsheetState[subsheet.document.id]
      ) {
            ...
            if (partState.scrollPositions?.length) {
              ...
              for (let i = 0; i < scrollableSelectors.length; i++) {
                ...
                const el1 =
                  selector === ""
                    ? subsheetElement
                    : subsheetElement.querySelector(selector);
                if (!el1) continue;
      ...
      if (subsheet.constructor.onSubmit)
        $('button[type="submit"]', subsheetElement)
          .attr("type", "button")
          .on("click", subsheet.constructor.onSubmit.bind(subsheet));
      $("form.journal-header", subsheetElement).on("submit", () => {
        return false;
      });
      ...
      if (!["blank", "folder"].includes(this.document.type)) {
        ...
        if (subsheet.document instanceof JournalEntryPage)
          Hooks.callAll(
            ...
            subsheet,
            subsheetElement,
            Object.assign({ enhancedjournal: this }, subsheetContext),
          );
      }
      ...
      this.activateControls(
        $("#left-journal-buttons", this.element).empty(),
        $("#right-journal-buttons", this.element).empty(),
      );
  ...
  _onChangeForm(formConfig, event) {
    if (
      ...
      return this.subsheet._onRevealSecret(event);
  ...
  _saveScrollPositions(html) {
    ...
    if (
      this.subsheet &&
      this.subsheet.rendered &&
      this.subsheet.options.scrollY &&
      this.subsheet.object.id == this.document.id
    ) {
      //only save if we're refreshing the sheet
      ...
      game.user.setFlag(
        "monks-enhanced-journal",
        `pagestate.${this.document.id}.scrollPositions`,
        foundry.utils.flattenObject(this._scrollPositions),
      );
    }
  }
  ...
  saveScrollPos() {
    if (
      ...
      let oldScrollPosition = foundry.utils.flattenObject(
        game.user.getFlag(
          "monks-enhanced-journal",
          `pagestate.${this.document.id}.scrollPositions`,
        ) || {},
      ...
      game.user.setFlag(
        ...
        foundry.utils.flattenObject(
          foundry.utils.mergeObject(oldScrollPosition, newScrollPositions),
        ),
      );
    }
  }
  ...
  saveEditor(name) {
    $(".nav-button.edit i", this.element)
      .addClass("fa-pencil-alt")
      .removeClass("fa-save")
      .attr("data-tooltip", i18n("MonksEnhancedJournal.EditDescription"));
    $(".nav-button.split", this.element).removeClass("disabled");
    const editor = this.subsheet.editors[name];
    if (editor) editor.button.style.display = "";

    const owner = this.document.isOwner;
    (game.system.id == "pf2e"
      ? game.pf2e.TextEditor
      : foundry.applications.ux.TextEditor.implementation
    )
      .enrichHTML(this.document.content, {
        secrets: owner,
        documents: true,
        async: true,
      })
  ...
  activateControls(left, right) {
    ...
    if (ctrls) {
      for (let ctrl of ctrls) {
        ...
        if (div != "") {
          if (ctrl.visible === false) div.hide();
          if (ctrl.leftAlign === true) left.append(div);
          else right.append(div);
        }
      }
    }
  }
  ...
  get entitytype() {
    if (this.document instanceof Actor) return "actor";

    let flags = this.document?.flags;
    let type =
      (flags != undefined ? flags["monks-enhanced-journal"]?.type : null) ||
      "oldentry";

    if (this.document?.folder?.name == "_fql_quests") type = "oldentry";
  ...
  async close(options) {
    if (options?.submit !== false) {
      ...
      if ((await this?.subsheet?.close()) === false) return false;
  ...
  canBack(tab) {
    if (tab == undefined) tab = this.tabs.active();
    if (tab == undefined) return false;
    return (
      tab.history?.length > 1 &&
      (tab.historyIdx == undefined || tab.historyIdx < tab.history.length - 1)
    );
  }
  ...
  async findEntity(entityId, text) {
    if (entityId == undefined)
      return new BlankJournal({
        flags: { "monks-enhanced-journal": { type: "blank" } },
        content: "",
      });
    else {
      let entity;
      if (entityId.indexOf(".") >= 0) {
        try {
          ...
          log("Error find entity", entityId, err);
        }
      } else {
        if (entity == undefined) entity = game.journal.get(entityId);
        if (entity == undefined) entity = game.actors.get(entityId);
      }
      if (entity == undefined)
        entity = new BlankJournal({
          name: text,
          flags: {
            "monks-enhanced-journal": { type: "blank" },
            content: `${i18n("MonksEnhancedJournal.CannotFindEntity")}: ${text}`,
          },
        });
  ...
  async deleteEntity(entityId) {
    ...
    for (let tab of this.tabs) {
      if (tab.entityId?.startsWith(entityId)) {
        ...
        $(
          '.journal-tab[data-tabid="${tab.id}"] .tab-content',
          this.element,
        ).html(tab.text);
      }
      ...
      if (tab.active && this.rendered) this.render(true); //if this entity was being shown on the active tab, then refresh the journal
    }
  ...
  addTab(entity, options = { activate: true, refresh: true }) {
    if (entity?.currentTarget != undefined) entity = null;
    ...
    let tab = {
      ...
      entity:
        ...
        new BlankJournal({
          flags: {
            "monks-enhanced-journal": { type: "blank" },
            content: i18n("MonksEnhancedJournal.NewTab"),
          },
        }),
      pageId: options.pageId,
      anchor: options.anchor,
    ...
    if (tab.entityId != undefined) tab.history.push(tab.entityId);
    this.tabs.push(tab);
    ...
    if (options.activate)
      ...
      this.saveTabs();
      if (options.refresh) this.render(true, { focus: true });
    }
  ...
  async activateTab(tab, event, options) {
    ...
    if (tab == undefined) tab = this.addTab();
    ...
    if (typeof tab == "string") tab = this.tabs.find((t) => t.id == tab);
    ...
    if (event?.altKey) {
      ...
      if (!tab) {
        if (this.tabs.length) tab = this.tabs[0];
        else tab = this.addTab();
      }
    }
    ...
    if (
      ...
      this.subsheetElement == undefined ||
      $(this.subsheetElement).is(":empty")
    ) {
    ...
    if (currentTab != undefined) currentTab.active = false;
    tab.active = true;

    if (this._tabs) this._tabs.active = null;
    ...
    //this.updateHistory();
    if (this.rendered) this.render(true, options);
    else {
      window.setTimeout(() => {
        $(`.journal-tab[data-tabid="${tab.id}"]`, this.element)
          .addClass("active")
          .siblings()
          .removeClass("active");
      }, 100);
  ...
  updateTab(tab, entity, options = {}) {
    ...
    if (tab != undefined) {
      if (tab.entityId != entity.uuid) {
        ...
        if (
          (game.user.isGM || setting("allow-player")) &&
          tab.entityId != undefined
        ) {
          //only save the history if the player is a GM or they get the full journal experience... and if it's not a blank tab
          if (tab.history == undefined) tab.history = [];
          ...
          if (tab.history.length > 10) tab.history = tab.history.slice(0, 10);
        }
  ...
  removeTab(tab, event) {
    ...
    if (this.tabs.length == 0) {
      ...
      if (tab.active) {
        ...
        if (!this.activateTab(nextIdx)) this.saveTabs();
      }
    }
  ...
  updateTabNames(uuid, name) {
    for (let tab of this.tabs) {
      if (tab.entityId == uuid) {
        $(`.journal-tab[data-tabid="${tab.id}"] .tab-content`, this.element)
          .attr("title", name)
          .html(name);
        tab.text = name;
        this.saveTabs();
  ...
  navigateHistory(event) {
    if (!$(event.currentTarget).hasClass("disabled")) {
      ...
      if (tab.history.length > 1) {
        ...
        do {
          idx =
            (tab.historyIdx == undefined ? 0 : tab.historyIdx) +
            (dir == "back" ? 1 : -1);
  ...
  async getHistory() {
    ...
    if (tab?.history == undefined) return;
    ...
    for (let i = 0; i < tab.history.length; i++) {
      ...
      if (tab?.entity?.id != undefined) {
        let type =
          entity.getFlag && entity.getFlag("monks-enhanced-journal", "type");
  ...
  addBookmark() {
    ...
    if (tab?.entityId == undefined) return;
    ...
    let entitytype = function (entity) {
      if (entity instanceof Actor) return "actor";
    ...
    $("<div>")
      ...
      .attr({
        ...
        "data-entity-id": bookmark.entityId,
      })
      .html(`<i class="fas ${bookmark.icon}"></i> ${bookmark.text}`)
      .appendTo(".bookmark-bar", this.element)
      .get(0)
      .click(this.activateBookmark.bind(this));
  ...
  removeBookmark(bookmark) {
    ...
    $(
      `.bookmark-button[data-bookmark-id="${bookmark.id}"]`,
      this.element,
    ).remove();
    this.saveBookmarks();
  }
  ...
  async open(entity, newtab, options) {
    ...
    if (this.tabs.length == 0) {
      ...
      if (newtab === true) {
        ...
        if (tab != undefined) this.activateTab(tab, null, options);
        else this.addTab(entity);
      } else {
        if ((await this?.subsheet?.close()) !== false) {
          // Check to see if this entity already exists in the tab list
          let tab = this.tabs.find((t) => t.entityId?.includes(entity.id));
          if (tab != undefined) this.activateTab(tab, null, options);
          else this.updateTab(this.tabs.active(), entity, options);
        }
      }
    }
  }
  ...
  async updateRecent(entity) {
    if (entity.id && entity.type != "blank") {
      let recent =
        game.user.getFlag("monks-enhanced-journal", "_recentlyViewed") || [];
      recent.findSplice((e) => e.id == entity.id || typeof e != "object");
      recent.unshift({
        id: entity.id,
        uuid: entity.uuid,
        name: entity.name,
        type: entity.getFlag("monks-enhanced-journal", "type"),
      });
  ...
  expandSidebar() {
    ...
    $(".sidebar-toggle", this.element).attr(
      "data-tooltip",
      i18n("MonksEnhancedJournal.CollapseDirectory"),
    );
    $(".sidebar-toggle i", this.element)
  ...
  collapseSidebar() {
    ...
    $(".sidebar-toggle", this.element).attr(
      "data-tooltip",
      i18n("MonksEnhancedJournal.ExpandDirectory"),
    );
    $(".sidebar-toggle i", this.element)
  ...
  searchText(query) {
    let that = this;
    $(
      ".editor-parent .editor.editor-display,.journal-entry-content",
      this.element,
    )
      .unmark()
      .mark(query, {
        ...
        noMatch: function () {
          ...
          if (query == "")
            $(".mainbar .navigation .search", that.element).removeClass(
              "error",
            );
          if (total > 0) {
            $(".mainbar .navigation .search", that.element).removeClass(
            ...
            let first = $(
              ".editor-parent .editor.editor-display mark:first,.journal-entry-content .scrollable mark:first",
              that.element,
            );
            $(".editor", that.element)
              .parent()
              .scrollTop(first.position().top - 10);
  ...
  _canDragStart(selector) {
    ...
    if (this.subsheet) return this.subsheet._canDragStart(selector);
    else return true; // super._canDragStart(selector);
  }

  _canDragDrop(selector) {
    if (this.subsheet) return this.subsheet._canDragDrop(selector);
    else return true;
  }
  ...
  _onDragStart(event) {
    ...
    if ($(target).hasClass("journal-tab")) {
      ...
      event.dataTransfer.setData("text/plain", JSON.stringify(dragData));
    } else return this.subsheet._onDragStart(event);
  }

  async _onDrop(event) {
    log("enhanced journal drop", event);
    let result = $(event.currentTarget).hasClass("enhanced-journal-header")
      ? false
      : this.subsheet._onDrop(event);
    ...
    if (result === false) {
      ...
      if (data.tabid) {
        ...
        if (from < to)
          $(
            '.journal-tab[data-tabid="' + data.tabid + '"]',
            this.element,
          ).insertAfter(target);
        else
          $(
        ...
        if (from < to)
          $(
            '.bookmark-button[data-bookmark-id="' + data.bookmarkId + '"]',
            this.element,
          ).insertAfter(target);
        else
          $(
        ...
        if (entity) this.open(entity, setting("open-new-tab"));
      }
  ...
  async _onSubmitForm(formConfig, event) {
    ...
    await this.subsheet.constructor.onSubmit.call(
      this.subsheet,
      event,
      form,
      formData,
  ...
  static doShowPlayers(event) {
    if (event.shiftKey)
      this.subsheet.constructor._onShowPlayers.call(
        this.subsheet,
        event,
        null,
        { users: null, showAs: "journal" },
      );
  ...
  fullscreen() {
    if (this.element.hasClass("maximized")) {
      ...
      $(".toggle-fullscreen", this.element).html(
        `<i class="fas fa-expand-arrows-alt"></i>${i18n("MonksEnhancedJournal.Maximize")}`,
      );
      this.setPosition({
        width: this._previousPosition.width,
        height: this._previousPosition.height,
      });
      this.setPosition({
        left: this._previousPosition.left,
        top: this._previousPosition.top,
      });
    } else {
      this.element.addClass("maximized");
      $(".toggle-fullscreen", this.element).html(
        `<i class="fas fa-compress-arrows-alt"></i>${i18n("MonksEnhancedJournal.Restore")}`,
      );
  ...
  async _createContextMenus(html) {
    this._context = new foundry.applications.ux.ContextMenu(
      html,
      ".bookmark-button",
      [
        {
          ...
          callback: async (li) => {
            let bookmark = this.bookmarks.find(
              (b) => b.id == li.dataset.bookmarkId,
            );
            ...
            if (document) {
              ...
              ui.notifications.warn(
                i18n("MonksEnhancedJournal.CannotFindEntity"),
              );
            }
          },
        },
    ...
    this._tabcontext = new foundry.applications.ux.ContextMenu(
      html,
      ".enhanced-journal-header .tab-bar",
      [
        {
          name: "Open outside Enhanced Journal",
          icon: '<i class="fas fa-file-export"></i>',
    ...
    $(".bookmark-bar .bookmark-button", this.element).on(
      "contextmenu",
      (event) => {
    ...
    this._historycontext = new foundry.applications.ux.ContextMenu(
      this.element,
      ".mainbar .navigation .nav-button.history",
      history,
      { fixed: true, jQuery: false },
    );
    this._imgcontext = new foundry.applications.ux.ContextMenu(
      this.element,
      ".journal-body.oldentry .tab.picture",
      [
        {
          name: "MonksEnhancedJournal.Delete",
          icon: '<i class="fas fa-trash"></i>',
  ...
  activateDirectoryListeners(html) {
    $(".sidebar-toggle", html).on("click", () => {
      if (this._collapsed) this.expandSidebar();
      else this.collapseSidebar();
    });

    ui.journal._createContextMenu(
    ...
    $(
      `[data-folder-depth="${this.maxFolderDepth}"] .create-folder`,
      html,
    ).remove();
    ...
    directory.on("click", ".folder-header", (event) => {
      ui.journal._onToggleFolder.call(
        this,
        event.originalEvent,
        event.currentTarget,
        { _skipDeprecation: true },
      );
    });
    ...
    if (ui.journal._canCreateFolder)
      $(".create-folder", html).click((event) => {
        ui.journal._onCreateFolder.call(
          ui.journal,
          event.originalEvent,
          event.currentTarget,
        );
      });
    if (ui.journal._canCreateEntry)
      $(".create-entry", html).click((event) => {
        ui.journal._onCreateEntry.call(
          ui.journal,
          event.originalEvent,
          event.currentTarget,
        );
    ...
    this._searchFilters = [
      new foundry.applications.ux.SearchFilter({
        ...
        callback: ui.journal._onSearchFilter.bind(ui.journal),
      }),
    ];
    ...
    html.querySelectorAll(".directory-item.folder").forEach((folder) => {
      folder.addEventListener(
        "dragenter",
        ui.journal._onDragHighlight.bind(ui.journal),
      );
      folder.addEventListener(
        "dragleave",
        ui.journal._onDragHighlight.bind(ui.journal),
  ...
  activateFooterListeners(html) {
    ...
    let content = folder
      ? folder.contents
      : ui.journal.collection.tree?.entries || ui.journal.documents;
    ...
    let documents = content
      .map((c) => {
        if (
          ...
          !c.testUserPermission(game.user, "OBSERVER")
        )
          return null;
        return {
          id: c.id,
    ...
    let idx = documents.findIndex(
      (e) => e.id == this.document.id || e.id == this.document.parent?.id,
    );

    let prev = idx > 0 ? documents[idx - 1] : null;
    let next = idx < documents.length - 1 ? documents[idx + 1] : null;
    $(".navigate-prev", html)
      .toggle(!["blank", "folder"].includes(this.document.type))
      .toggleClass("disabled", !prev)
      .attr("data-tooltip", prev?.name);
    $(".navigate-next", html)
      .toggle(!["blank", "folder"].includes(this.document.type))
      .toggleClass("disabled", !next)
      .attr("data-tooltip", next?.name);
  ...
  getDocuments() {
    let folder = this.document.folder || this.document.parent?.folder;
    let contents =
      folder?.contents || game.journal.filter((j) => j.folder == null);

    let sortingMode =
      folder?.sorting || ui.journal.collection.sortingMode || "m";
