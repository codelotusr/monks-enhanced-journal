...
export class LootSheet extends EnhancedJournalSheet {
  ...
  static PARTS = {
    main: {
      ...
      scrollable: [".items-list .item-list"],
    },
  };
  ...
  async _prepareBodyContext(context, options) {
    ...
    if (actors == undefined && game.user.isGM) {
      ...
      for (let user of game.users) {
        if (
          user.character &&
          (user.active || setting("loot-inactive-players"))
        ) {
          actors[user.character.id] = {
    ...
    context.canRequest =
      context.data.flags["monks-enhanced-journal"].purchasing == "locked";

    context.characters = actorIds
      .map((a) => {
  ...
  configure() {
    ...
    if (document instanceof JournalEntryPage) document = document.parent;
    new DocumentOwnershipConfig(document).render(true);
  }

  static async onSplitCurrency(event, target) {
    let characters = Object.keys(
      this.document.getFlag("monks-enhanced-journal", "actors") || {},
    )
      .map((a) => {
    ...
    if (characters.length == 0) {
      ui.notifications.warn(
        i18n("MonksEnhancedJournal.msg.ThereAreNoCharactersToDistribute"),
      );
      return;
    }

    let currency = foundry.utils.duplicate(
      this.document.getFlag("monks-enhanced-journal", "currency") || {},
    );
    if (Object.values(currency).find((v) => v > 0) == undefined) {
      ui.notifications.warn(
        i18n("MonksEnhancedJournal.msg.ThereAreNoMoneyToDistribute"),
      );
      return;
    }

    new DistributeCurrency({ characters, currency, loot: this }).render(true, {
      focus: true,
    });
  }
  ...
  transferCurrency(actor, event) {
    new TransferCurrency({ document: this.document, actor, loot: this }).render(
      true,
      { focus: true },
    );
  }
  ...
  _prepareSubmitData(event, form, formData, updateData) {
    let submitData = super._prepareSubmitData(
      event,
      form,
      formData,
      updateData,
    ...
    if (this.document.isOwner) {
      ...
      let items = foundry.utils.duplicate(
        submitData.flags["monks-enhanced-journal"].items || {},
      );
  ...
  _canDragItemStart(selector) {
    return (
      game.user.isGM || this.document.testUserPermission(game.user, "OBSERVER")
    );
  }
  ...
  async _onDragItemsStart(event) {
    ...
    if (
      item == undefined ||
      (!game.user.isGM &&
        (item?.lock === true || getValue(item, quantityname()) <= 0))
    )
      return;
    ...
    if (data.type == "Item") {
      ...
      if (data.from == this.document.uuid) {
        ...
        if (actor) {
          ...
          let result = await LootSheet.confirmQuantity(
            item,
            max,
            "transfer",
            false,
          ...
          if ((result?.quantity ?? 0) > 0) {
            ...
            try {
              ...
              console.warn(
                "[Monks Loot SF2e Patch] _onDropLootItem failed, using fallback",
                err,
              );
              await actor.createEmbeddedDocuments("Item", [itemData]);
            }
            ...
            if (entry)
              this.constructor.purchaseItem.call(
                this.constructor,
                entry,
                data.data._id,
                result.quantity,
        ...
        if (!this.document.isOwner && !hasGM) {
          return ui.notifications.warn(
            "Cannot drop items on this sheet without a GM logged in",
          );
        }
        if (data.groupSelect) {
          ...
          if (!entry && !item.actor?.id) max = null;
          ...
          if (
            ...
            !(
              ...
              (entry.flags["monks-enhanced-journal"].purchasing == "free" ||
                entry.isOwner)
            )
          )
            return;
          ...
          if (!this.document.isOwner && !(item.actor.id || entry)) return;
          ...
          if ((result?.quantity ?? 0) > 0) {
            ...
            if (entry) {
              ...
              if (result.quantity >= quantity) await item.delete();
              else {
                let update = { system: {} };
  ...
  async _onDropActor(event, target) {
    ...
    if (data.type == "Actor") {
      ...
      let actors = foundry.utils.duplicate(
        this.document.getFlag("monks-enhanced-journal", "actors") || {},
      );
  ...
  static async onRequestItem(event, target) {
    ...
    if (
      ...
      ui.notifications.warn(
        i18n("MonksEnhancedJournal.msg.CannotTakeLootWithoutGM"),
      );
      return false;
    }
    ...
    if (this.document.flags["monks-enhanced-journal"].purchasing == "locked") {
      ...
      if ((result?.quantity ?? 0) > 0) {
        ...
        LootSheet.createRequestMessage.call(
          this,
          this.document,
          item,
          actor,
      ...
      if ((result?.quantity ?? 0) > 0) {
        ...
        try {
          ...
          console.warn(
            "[Monks Loot SF2e Patch] _onDropItem failed, using fallback",
            err,
          );
          try {
            await actor.createEmbeddedDocuments("Item", [itemData]);
            ...
            console.error(
              "[Monks Loot SF2e Patch] CRITICAL: Item grant failed",
              fatalErr,
            );
  ...
  static async onGrantItem(event, target) {
    ...
    if (
      !item ||
      foundry.utils.getProperty(item, "flags.monks-enhanced-journal.requests")
        .length == 0
    )
      return;
    ...
    let result = await LootSheet.confirmQuantity(
      item,
      max,
      format("MonksEnhancedJournal.GrantToActor", { name: actor.name }),
      false,
    );
    ...
    if ((result?.quantity ?? 0) > 0) {
      foundry.utils.setProperty(
        item,
        "flags.monks-enhanced-journal.requests." + userId,
        false,
      );
      ...
      try {
        ...
        console.warn(
          "[Monks Loot SF2e Patch] _onDropItem failed, using direct create fallback",
          err,
        );
        try {
          ...
          console.error(
            "[Monks Loot SF2e Patch] CRITICAL: Failed to grant item.",
            fatalErr,
          );
          ui.notifications.error(
            "Failed to grant item. Check console for details.",
          );
        }
      }

      // Record the purchase/grant in the journal
      await this.constructor.purchaseItem.call(
        this.constructor,
        this.document,
        id,
        result.quantity,
  ...
  static async itemDropped(id, actor, entry) {
    ...
    if (item) {
      ...
      if ((result?.quantity ?? 0) > 0) {
        if (game.user.isGM) {
          LootSheet.purchaseItem.call(this, entry, id, result.quantity, {
            actor,
          });
          return result;
        } else {
          if (
            entry.data.flags["monks-enhanced-journal"].purchasing == "confirm"
          ) {
