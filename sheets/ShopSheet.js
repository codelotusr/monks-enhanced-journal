...
export class ShopSheet extends EnhancedJournalSheet {
  ...
  async _prepareBodyContext(context, options) {
    ...
    if (
      !foundry.utils.hasProperty(
        context,
        "data.flags.monks-enhanced-journal.sheet-settings.adjustment",
      ) &&
      foundry.utils.hasProperty(
        context,
        "data.flags.monks-enhanced-journal.adjustment",
      )
    ) {
    ...
    context.groups = await this.getItemGroups(
      foundry.utils.getProperty(
        context,
        "data.flags.monks-enhanced-journal.purchasing",
      ),
      this.document._sort,
    );

    let purchasing =
      context.data.flags["monks-enhanced-journal"]?.purchasing || "confirm";
    let hasGM = game.users.find((u) => u.isGM && u.active) != undefined;
    context.showrequest =
      ["confirm", "free"].includes(purchasing) &&
    ...
    if (actorLink) {
      let actor = actorLink.id
        ? game.actors.find((a) => a.id == actorLink.id)
        : await fromUuid(actorLink);
    ...
    context.hasRollTables = !!game.packs.get(
      "monks-enhanced-journal.shop-names",
    );

    let getTime = (prop) => {
      let twentyfour = foundry.utils.getProperty(
        context,
        `data.flags.monks-enhanced-journal.twentyfour`,
      );
      let time = foundry.utils.getProperty(
        context,
        `data.flags.monks-enhanced-journal.${prop}`,
      );
      let hours = Math.floor(time / 60);
      let minutes = Math.trunc(time - hours * 60);
      return time
        ? `${twentyfour || hours < 13 ? hours : hours - 12}:${String(minutes).padStart(2, "0")}${!twentyfour ? " " + (hours >= 12 ? "PM" : "AM") : ""}`
        : "";
    };
    ...
    context.hours =
      context.opening && context.closing
        ? `${context.opening} - ${context.closing}, `
        : "";

    let state = foundry.utils.getProperty(
      context,
      "data.flags.monks-enhanced-journal.state",
    );
    ...
    context.log = (
      foundry.utils.getProperty(
        context,
        "data.flags.monks-enhanced-journal.log",
      ) || []
    ).map((l) => {
    ...
    context.fields = [
      {
        id: "shoptype",
        label: "MonksEnhancedJournal.ShopType",
        value: foundry.utils.getProperty(
          context.data,
          "flags.monks-enhanced-journal.shoptype",
        ),
      },
      {
        id: "location",
        label: "MonksEnhancedJournal.Location",
        value: foundry.utils.getProperty(
          context.data,
      ...
      {
        label: "MonksEnhancedJournal.HoursOfOperation",
        value: `${context.hours} ${context.open ? i18n("MonksEnhancedJournal.Open") : i18n("MonksEnhancedJournal.Closed")}`,
      },
    ];
    context.placeholder = "MonksEnhancedJournal.ShopName";

    context.hasShowToPlayers = true;
    context.showingToPlayers =
      this.document.parent.ownership["default"] >=
  ...
  _documentControls() {
    let ctrls = [
      { label: '<i class="fas fa-search"></i>', type: "text" },
      {
        id: "search",
        type: "input",
        label: i18n("MonksEnhancedJournal.SearchDescription"),
        visible: !!this.enhancedjournal,
        callback: this.searchText,
      },
      {
        id: "show",
        label: i18n("MonksEnhancedJournal.ShowToPlayers"),
        icon: "fas fa-eye",
      ...
      {
        id: "edit",
        label: i18n("MonksEnhancedJournal.EditDescription"),
      ...
      {
        id: "sound",
        label: i18n("MonksEnhancedJournal.AddSound"),
        icon: "fas fa-music",
        visible: this.isEditable,
        action: "addSound",
      },
      {
        id: "convert",
        label: i18n("MonksEnhancedJournal.Convert"),
        icon: "fas fa-clipboard-list",
        visible: game.user.isGM && this.isEditable,
        action: "convertSheet",
      },
    ];
    ...
    let parseTime = (prop) => {
      if (fd[prop]) {
        ...
        foundry.utils.setProperty(
          submitData,
          "flags.monks-enhanced-journal.twentyfour",
          !ampm,
        );
        foundry.utils.setProperty(
          submitData,
          `flags.monks-enhanced-journal.${prop}`,
  ...
  _canDragItemStart(selector) {
    ...
    return (
      game.user.isGM ||
      (["free", "confirm"].includes(
        this.document.flags["monks-enhanced-journal"].purchasing,
      ) &&
        hasGM)
    );
  }
    ...
    if (target.dataset.document == "Actor") {
      ...
      if (!game.user.isGM && qty != null && qty <= 0) {
        ui.notifications.warn(
          i18n("MonksEnhancedJournal.msg.NotEnoughRemainsToBeTransferred"),
        );
        return;
      }
      ...
      if (folder) {
        ...
        for (let item of folder.contents) {
          if (item instanceof Item) {
            ...
            foundry.utils.setProperty(
              itemData,
              "flags.monks-enhanced-journal.price",
      ...
      if (data.groupSelect) {
        ...
        if (item.parent instanceof Actor) {
          ...
          if (game.user.isGM) {
            ...
            let buy =
              adjustment[item.type]?.buy ?? adjustment.default.buy ?? 0.5;
            if (buy == -1)
              return ui.notifications.warn(
                i18n("MonksEnhancedJournal.msg.CannotSellItem"),
              );
            price.value = Math.floor(price.value * buy);
            let result = await this.constructor.confirmQuantity(
              item,
              max,
              "sell",
              true,
            ...
            if ((result?.quantity ?? 0) > 0) {
              ...
              foundry.utils.setProperty(
                itemData,
                "flags.monks-enhanced-journal.price",
                origPrice + " " + price.currency,
              );
              foundry.utils.setProperty(
                itemData,
                "flags.monks-enhanced-journal.lock",
                true,
              );
              foundry.utils.setProperty(
                itemData,
              ...
              await this.constructor.actorPurchase(actor, {
                value: -(result.price.value * result.quantity),
                currency: result.price.currency,
              });

              if (result.quantity >= max) item.delete();
              else {
                let update = { system: {} };
                foundry.utils.setProperty(
                  update.system,
              ...
              this.constructor.addLog.call(this.document, {
                actor: actor.name,
                item: item.name,
            ...
            let selling = this.document.getFlag(
              "monks-enhanced-journal",
              "selling",
            );
            if (selling == "locked" || !selling) {
              ui.notifications.warn(
                i18n("MonksEnhancedJournal.msg.ShopIsNotReceivingItems"),
              );
              return false;
            }
            ...
            if (!hasGM) {
              ui.notifications.warn(
                i18n("MonksEnhancedJournal.msg.CannotSellItemWithoutGM"),
              );
              return false;
            }
            //request to sell
            ...
            if ((result?.quantity ?? 0) > 0) {
              if (selling == "free") {
                ...
                await this.constructor.actorPurchase(actor, {
                  value: -(price.value * result.quantity),
          ...
          let result = await ShopSheet.confirmQuantity(
            item,
            null,
            "transfer",
            false,
      ...
      if (
        ...
        let shopPage =
          this.document instanceof JournalEntry
            ...
            : this.document;
        let oldItems = foundry.utils.duplicate(
          foundry.utils.getProperty(
            shopPage,
            "flags.monks-enhanced-journal.items",
          ) || [],
        ...
        if (oldItems.length) {
          await foundry.applications.api.DialogV2.wait({
            ...
            content:
              "Would you like to replace the items in the shop with these items, or add to the items already in the shop?",
            focus: true,
            default: "replace",
            close: () => {
              return true;
  ...
  static async onAdjustPrice(event) {
    new AdjustPrice({ document: this.document, journalsheet: this }).render(
      true,
    );
  }
  ...
  static async onRequestItem(event, target) {
    ...
    if (!actor) {
      ui.notifications.warn(
        i18n("MonksEnhancedJournal.msg.YouDontHaveCharacter"),
      );
      return;
    }
    ...
    if (data.cost && data.cost != "") {
      ...
      if (!this.constructor.canAfford(item, actor)) {
        ui.notifications.warn(
          format("MonksEnhancedJournal.msg.CannotTransferCannotAffordIt", {
            name: actor.name,
          }),
        );
        return false;
    ...
    if (!hasGM) {
      ui.notifications.warn(
        i18n("MonksEnhancedJournal.msg.CannotPurchaseItemWithoutGM"),
      );
      return false;
    }
    ...
    if ((result?.quantity ?? 0) > 0) {
      ...
      if (
        this.document.flags["monks-enhanced-journal"].purchasing == "confirm"
      ) {
        ...
        foundry.utils.setProperty(
          item,
          "flags.monks-enhanced-journal.maxquantity",
          max != "" ? parseInt(max) : null,
        );

        if (
          !ShopSheet.canAfford(
            result.quantity * price.value + " " + price.currency,
            actor,
          )
        )
          ui.notifications.error(
            format("MonksEnhancedJournal.msg.ActorCannotAffordItem", {
              name: actor.name,
              quantity: result.quantity,
              itemname: item.name,
            }),
          ...
          this.constructor.createRequestMessage.call(
            this,
            this.document,
            item,
            actor,
          ...
          MonksEnhancedJournal.emit("notify", {
            actor: actor.name,
            item: item.name,
          });
        }
      } else if (
        this.document.flags["monks-enhanced-journal"].purchasing == "free"
      ) {
  ...
  async createSellMessage(item, actor) {
    ...
    let content = await foundry.applications.handlebars.renderTemplate(
      "./modules/monks-enhanced-journal/templates/request-sale.html",
      messageContent,
    );
    let messageData = {
      user: game.user.id,
      ...
      flavor: speaker.alias
        ? format("MonksEnhancedJournal.ActorWantsToPurchase", {
            alias: speaker.alias,
            verb: i18n("MonksEnhancedJournal.Sell").toLowerCase(),
          })
        : null,
      whisper: whisper,
      flags: {
  ...
  static onClickItem(event, target) {
    ...
    if (
      item == undefined &&
      this.document.flags["monks-enhanced-journal"].actor
    ) {
      let actorid = this.document.flags["monks-enhanced-journal"].actor.id;
      let actor = game.actors.get(actorid);
      if (actor) item = actor.items.get(li.dataset.id);
    }
  ...
  static canAfford(item, actor) {
    ...
    if (MonksEnhancedJournal.currencies.length == 0) return true;
    ...
    if (setting("purchase-conversion")) {
      ...
      if (coins >= price.value) {
        ...
        for (let curr of MonksEnhancedJournal.currencies) {
          totalDefault +=
            this.getCurrency(actor, curr.id) * (curr.convert || 1);
        }
        let check = MonksEnhancedJournal.currencies.find(
          (c) => c.id == price.currency,
        );
  ...
  static actorPurchase(actor, price) {
    ...
    if (MonksEnhancedJournal.currencies.length == 0) return;
  ...
  static async itemDropped(id, actor, entry) {
    ...
    if (item) {
      let max = foundry.utils.getProperty(
        item,
        "flags.monks-enhanced-journal.quantity",
      );
      if (!game.user.isGM && max != null && max <= 0) {
      ...
      let cost = foundry.utils.getProperty(
        item,
      ...
      if ((result?.quantity ?? 0) > 0) {
        price = result.price;
        if (game.user.isGM) {
          ShopSheet.actorPurchase.call(entry, actor, {
            value: price.value * result.quantity,
            currency: price.currency,
          });
          ShopSheet.purchaseItem.call(this, entry, id, result.quantity, {
            actor,
            purchased: true,
          });
          if (item.consumable) result.quantity = 0;
          this.addLog.call(entry, {
            actor: actor.name,
            item: item.name,
            quantity: result.quantity,
            price: result.price.value + " " + result.price.currency,
            type: "purchase",
          });
          return result;
        } else {
          if (
            foundry.utils.getProperty(
              entry,
              "flags.monks-enhanced-journal.purchasing",
            ) == "confirm"
          ) {
            //create the chat message informaing the GM that player is trying to sell an item.
            foundry.utils.setProperty(
            ...
            if (
              ...
              ShopSheet.createRequestMessage.call(
                this,
                entry,
                item,
                actor,
        ...
        log("result", result);
        ui.notifications.warn(
          i18n("MonksEnhancedJournal.msg.CannotAddLessThanOne"),
        );
      }
    }
    return false;
        ...
        let items = actor.items
          .filter((item) => {
            ...
            // Equipment's fine, unless it's natural armor
              ...
              if (!item.system.armor) result = true;
              else result = item.system.armor.type != "natural";
            } else
              result = ![
                "class",
                "spell",
          ...
          .map((i) => {
            return foundry.utils.mergeObject(i.toObject(), {
              cost: getValue(i.data, pricename(), ""),
            });
          });

        if (items.length > 0) {
          let shopitems = foundry.utils.duplicate(
  ...
  async convertItems(formData) {
    for (let [k, v] of Object.entries(formData.adjustment)) {
      if (v.sell == undefined) delete formData.adjustment[k].sell;
      if (v.buy == undefined) delete formData.adjustment[k].buy;
    ...
    let adjustment = foundry.utils.mergeObject(
      settingDefaults,
      formData.adjustment || {},
    );

    let items =
      this.options.document.getFlag("monks-enhanced-journal", "items") || {};
    ...
    for (let item of Object.values(items)) {
      ...
      let cost =
        Math.max(Math.ceil(price.value * sell, 1)) + " " + price.currency;
      foundry.utils.setProperty(
        item,
        "flags.monks-enhanced-journal.cost",
        cost,
