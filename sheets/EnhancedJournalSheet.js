...
export class EnhancedJournalSheet extends HandlebarsApplicationMixin(
  ...
  constructor(options = {}) {
    ...
    try {
      this._scrollPositions = JSON.parse(
        this.document.flags["monks-enhanced-journal"]?.scrollPos || {},
      );
    } catch (e) {}
  }
  ...
  _initializeApplicationOptions(options) {
    ...
    if (!setting("show-bookmarkbar")) options.classes.push("hide-bookmark");
  ...
  get classes() {
    ...
    for (const cls of this.constructor.inheritanceChain()) {
      if (cls.hasOwnProperty("DEFAULT_OPTIONS"))
        classes = classes.concat(
          cls.DEFAULT_OPTIONS.window?.contentClasses || [],
        );
    }
    return classes;
  }
  ...
  get actions() {
    ...
    for (const cls of this.constructor.inheritanceChain()) {
      if (
        cls.hasOwnProperty("DEFAULT_OPTIONS") &&
        cls.DEFAULT_OPTIONS.actions
      ) {
  ...
  get trueElement() {
    if (this.enhancedjournal) return this.enhancedjournal.subsheetElement;
    return this.element;
  }

  static sheetSettings() {
    let settingDefault =
      (game.settings.settings.get("monks-enhanced-journal.sheet-settings")
        ?.default || {})[this.type] || {};
    let sheetSettings = (setting("sheet-settings") || {})[this.type] || {};
    return foundry.utils.mergeObject(
      foundry.utils.duplicate(settingDefault),
      foundry.utils.duplicate(sheetSettings),
    );
  }
  ...
  sheetSettings() {
    ...
    let settings =
      foundry.utils.getProperty(
        this.document,
        "flags.monks-enhanced-journal.sheet-settings",
      ) || {};
    return foundry.utils.mergeObject(
      sheetSettings,
      foundry.utils.duplicate(settings),
    );
  }
  ...
  _canUserView(user) {
    if (this.document.compendium)
      return user.isGM || this.document.compendium.visible;
    return this.document.parent.testUserPermission(
      user,
      this.options.viewPermission,
    );
  }
  ...
  _getHeaderButtons() {
    ...
    if (!canConfigure) buttons.findSplice((b) => b.class == "configure-sheet");
  ...
  _getHeaderControls() {
    if (this.enhancedjournal) return [];
    return this._documentControls().filter(
      (c) => !c.type || c.type == "button",
    );
  }
  ...
  async _prepareBodyContext(context, options) {
    ...
    context.userdata = foundry.utils.getProperty(
      context.source,
      `flags.monks-enhanced-journal.${context.user.id}`,
    );
    if (context.userdata) {
      context.userdata.enrichedText =
        await foundry.applications.ux.TextEditor.implementation.enrichHTML(
    ...
    if (this.canPlaySound) {
      context.sound =
        foundry.utils.getProperty(
          context.source,
          "flags.monks-enhanced-journal.sound",
        ) || {};
      if (this.enhancedjournal)
        context.sound.playing = (this.enhancedjournal._backgroundsound || {})[
          this.document.id
        ]?.playing;
      else context.sound.playing = this._backgroundsound?.playing;
    }
  ...
  static async convertSheet(event, target) {
    let context = {
      options: [
        {
          id: "encounter",
          name: "MonksEnhancedJournal.sheettype.encounter",
          disabled: this.constructor.type == "encounter",
        },
        {
          id: "event",
          name: "MonksEnhancedJournal.sheettype.event",
          disabled: this.constructor.type == "event",
        },
        {
          id: "loot",
          name: "MonksEnhancedJournal.sheettype.loot",
          disabled: this.constructor.type == "loot",
        },
        {
          id: "organization",
          name: "MonksEnhancedJournal.sheettype.organization",
          disabled: this.constructor.type == "organization",
        },
        {
          id: "person",
          name: "MonksEnhancedJournal.sheettype.person",
          disabled: this.constructor.type == "person",
        },
        {
          id: "place",
          name: "MonksEnhancedJournal.sheettype.place",
          disabled: this.constructor.type == "place",
        },
        {
          id: "poi",
          name: "MonksEnhancedJournal.sheettype.poi",
          disabled: this.constructor.type == "poi",
        },
        {
          id: "quest",
          name: "MonksEnhancedJournal.sheettype.quest",
          disabled: this.constructor.type == "quest",
        },
        {
          id: "shop",
          name: "MonksEnhancedJournal.sheettype.shop",
          disabled: this.constructor.type == "shop",
        },
        {
          id: "picture",
          name: "MonksEnhancedJournal.sheettype.picture",
          disabled: this.constructor.type == "picture",
        },
        {
          id: "text",
          name: "MonksEnhancedJournal.sheettype.journalentry",
          disabled: this.constructor.type == "text",
        },
      ],
      sheetType: i18n(
        `MonksEnhancedJournal.sheettype.${this.constructor.type}`,
      ),
    };
    let html = await foundry.applications.handlebars.renderTemplate(
      "modules/monks-enhanced-journal/templates/convert.html",
      context,
    );
    let that = this;
  ...
  get isEditable() {
    if (this.enhancedjournal && !this.enhancedjournal.isEditable) return false;

    return (
      this.document.permission == CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER &&
      this.document?.compendium?.locked !== true
    );
  }
    ...
    if (
      ...
      (!this.document.testUserPermission(game.user, "OBSERVER") ||
        (this.document.parent &&
          !this.document.parent.testUserPermission(game.user, "OBSERVER")))
    ) {
      this.document.ownership[game.user.id] =
        CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER;
      if (this.document.parent)
        this.document.parent.ownership[game.user.id] =
          CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER;
      this.tempOwnership = true;
    }
    if (this.enhancedjournal) this.enhancedjournal.render(options);
    else {
  ...
  async subRender(context, options) {
    ...
    if (hasGM) {
      ...
      let editor = $(".notes-container prose-mirror.editor").on(
        "open",
        (ev) => {
  ...
  async activateListeners(html) {
    $("[data-blur-action]", html).on(
      "blur",
      EnhancedJournalSheet.#onAction.bind(this, "blur"),
    );
    $("[data-keypress-action]", html).on(
      "keypress",
      EnhancedJournalSheet.#onAction.bind(this, "keypress"),
    );
    $(
      "[data-change-action],[data-change-action] > input[type='text']",
      html,
    ).on("change", EnhancedJournalSheet.#onAction.bind(this, "change"));
    $("[data-click-action]", html).on(
      "click",
      EnhancedJournalSheet.#onAction.bind(this, "click"),
    );
    $("[data-dblclick-action]", html).on(
      "dblclick",
      EnhancedJournalSheet.#onAction.bind(this, "dblclick"),
    );
    $("[data-contextmenu-action]", html).on(
      "contextmenu",
      EnhancedJournalSheet.#onAction.bind(this, "contextmenu"),
    );

    $(".actor-img img", html).on(
      "dragstart",
      foundry.applications.ux.TextEditor.implementation._onDragContentLink,
    );
    ...
    $("a.picture-link", html).click(
      MonksEnhancedJournal._onClickPictureLink.bind(this),
    );
    ...
    $("header.collapsible", html).on(
      "click",
      this.collapseItemSection.bind(this),
    );

    $("div[data-tab='description'] prose-mirror.editor", html).on(
      "save",
      (ev) => {},
    );
  ...
  activateEditor(name, options = {}, initialContent = "") {
    ...
    if (this.editors[name] != undefined) {
      ...
      if (
        ...
        this.document.type == "journalentry" ||
        this.document.type == "oldentry" ||
      ...
      else super.activateEditor(name, options, initialContent);
      ...
      let timer = window.setInterval(function () {
        ...
        if (editor && editor.mce) {
          ...
          that.constructor.updateStyle.call(
            that,
            data,
            $(editor.mce.contentDocument),
          );
          window.clearInterval(timer);
        }
      }, 100);
  ...
  _contextMenu(html) {
    new foundry.applications.ux.ContextMenu(
      html,
      ".editor-parent",
      this._getDescriptionContextOptions(),
      { fixed: true, jQuery: false },
    );

    this.pictureContext = new foundry.applications.ux.ContextMenu(
      html,
      "img[data-edit],div.picture-img",
      this._getImageContextOptions(),
      { eventName: "manual", fixed: true, jQuery: false },
    );

    new foundry.applications.ux.ContextMenu(
      html,
      ".actor-img-container",
      this._getPersonActorContextOptions(),
      { fixed: true, jQuery: false },
    );
  }
  ...
  async getRelationships() {
    let relationships =
      this.document.flags["monks-enhanced-journal"]?.relationships || {};
    ...
    for (let [relationshipId, relationship] of Object.entries(relationships)) {
      let entity = relationship.uuid
        ? await fromUuid(relationship.uuid)
        : game.journal.get(relationshipId);
      if (
        !(entity instanceof JournalEntry || entity instanceof JournalEntryPage)
      )
        continue;
      if (
        entity &&
        entity.testUserPermission(game.user, "LIMITED") &&
        (game.user.isGM || !relationship.hidden)
      ) {
        let page =
        ...
        let type = foundry.utils.getProperty(
          page,
        ...
        if (!results[type])
          results[type] = {
            ...
            name:
              type &&
              (
                game.i18n.translations.MonksEnhancedJournal ||
                game.i18n._fallback.MonksEnhancedJournal ||
                {}
              )?.sheettype[type?.toLowerCase()]
                ? i18n(`MonksEnhancedJournal.sheettype.${type?.toLowerCase()}`)
                : i18n("MonksEnhancedJournal.Unknown"),
            documents: [],
          };

        if (
          results[type].documents.some(
            (r) => r.id == relationship.id && r.uuid == relationship.uuid,
          )
        )
          continue;
        ...
        relationship.img =
          page.src || `modules/monks-enhanced-journal/assets/${type}.png`;
        relationship.type = type;
        relationship.shoptype = page.getFlag(
          "monks-enhanced-journal",
          "shoptype",
        );
            ...
            yes: {
              callback: async () => {
                ...
                if (this.constructor.type == "picture") {
                  $('img[data-edit="src"],div.picture-img', this.trueElement)
                    .css({ opacity: 0 })
                    .attr("src", "")
                    .css({ backgroundImage: "" });
                  $(
                    ".sheet-body .instruction, .tab.picture .instruction",
                    this.trueElement,
                  ).show();
                } else {
                  let defaultImage =
                    this.constructor.type == "picture"
                      ? ""
                      : `modules/monks-enhanced-journal/assets/${this.constructor.type}.png`;
                  $('img[data-edit="src"],div.picture-img', this.trueElement)
                    .attr("src", defaultImage)
                    .css({ backgroundImage: defaultImage });
                }
              },
            },
          });
  ...
  _getPersonActorContextOptions() {
    return [
      ...
      },
      {
        ...
        callback: () => {
          foundry.applications.api.DialogV2.confirm({
            ...
            content: i18n(
              "MonksEnhancedJournal.msg.ConfirmImportAllItemsToShop",
            ),
      ...
      },
      {
        ...
        icon: '<i class="fas fa-image"></i>',
        callback: () => {
          let actorLink = this.document.getFlag(
            "monks-enhanced-journal",
            "actor",
          );
          ...
          ip.shareImage = () =>
            foundry.documents.collections.Journal.showDialog(actor, {
              showAs: "image",
            });
          ip.render(true);
        },
  ...
  _disableFields(form) {
    ...
    if (hasGM) {
      ...
      $(
        `textarea[name="flags.monks-enhanced-journal.${game.user.id}.notes"]`,
        form,
      )
        .removeAttr("disabled")
        .removeAttr("readonly")
        .on("blur", this._onChangeInput.bind(this));
    }
  ...
  static getCurrency(actor, denomination) {
    ...
    switch (game.system.id) {
      case "pf2e":
        {
          let coin = actor.items.find((i) => {
            return i.isCoinage && i.system.price.value[denomination] == 1;
          });
      ...
      case "dsa5":
        {
          let coin = actor.items.find((i) => {
            return i.type == "money" && i.name == denomination;
          });
      ...
      case "wfrp4e":
        {
          let currency = MonksEnhancedJournal.currencies.find(
            (c) => c.id == denomination,
          );
          let coin = actor.itemCategories.money.find((i) => {
            return i.type == "money" && i.name == currency.name;
          });
      ...
      case "mythras":
        {
          ...
          let coin = actor.items.find((i) => {
            return i.type == "currency" && i.name == currency.name;
          });
      ...
      case "cyphersystem":
        {
          ...
          let systemcoins = {
            ...
            name:
              coins.numberCategories == "1"
                ? i18n("CYPHERSYSTEM.Shins")
                : i18n("CYPHERSYSTEM.Copper"),
          };

          let coinname =
            Object.keys(coins).find((key) => coins[key] == currency.name) ||
            Object.keys(systemcoins).find(
              (key) => systemcoins[key] == currency.name,
            );
      ...
      case "sfrpg":
        coinage = parseInt(
          actor.system.currency[denomination == "cr" ? "credit" : denomination],
        );
        break;
      case "pirateborg":
        coinage = parseInt(actor.system[denomination]);
      ...
      default:
        {
          let coin =
            currencyname() == ""
              ? actor
              : (getValue(actor, currencyname()) ?? actor);
  ...
  static async addCurrency(actor, denomination, value) {
    ...
    if (value < 0 && setting("purchase-conversion")) {
      let currencies = foundry.utils
        .duplicate(MonksEnhancedJournal.currencies || [])
        .filter((c) => c.convert != undefined);
      ...
      while (remainder > 0 && idx >= 0) {
        ...
        if (idx >= 0 && currencies[idx].value > 0) {
          let rate =
            (currencies[idx].convert || 1) /
            (currencies[denomIdx].convert || 1);
      ...
      for (let curr of Object.keys(changes)) {
        ...
        if (changes[curr] == orig.value) delete changes[curr];
      }
    } else
      changes[denomination] =
        parseInt(this.getCurrency(actor, denomination) || 0) + value;

    let updates = {};
    if (game.system.id == "pf2e") {
      let promises = [];
      for (let [k, v] of Object.entries(changes)) {
        let coinage = actor.items.find((i) => {
          return i.isCoinage && i.system.price.value[k] == 1;
        });
        if (!coinage) {
          ...
          if (items.length) coinage = items[0];
        } else {
      ...
      for (let [k, v] of Object.entries(changes)) {
        ...
        let coinage = actor.items.find((i) => {
          return i.type == "currency" && i.name == currency;
        });
        if (coinage) {
          updates[`system.quantity`] = v;
          promises.push(coinage.update(updates));
      ...
      for (let [k, v] of Object.entries(changes)) {
        let currency = MonksEnhancedJournal.currencies.find((c) => c.id == k);
        let coinage = actor.itemCategories.money.find((i) => {
          return i.type == "money" && i.name == currency.name;
        });
        if (!coinage) {
          ...
          default:
            {
              let coin =
                currencyname() == ""
                  ...
                  : (getValue(actor, currencyname()) ?? actor);
              updates[
                `system${currencyname() != "" ? "." : ""}${currencyname()}.${k}`
              ] = coin[k] && coin[k].hasOwnProperty("value") ? { value: v } : v;
            }
            break;
        }
      }
  ...
  static onAddSound(event, target) {
    let sound = this.enhancedjournal
      ? this.enhancedjournal._backgroundsound[this.document.id]
  ...
  loadSound(src, autoplay, autoplayOptions) {
    ...
    else this._backgroundsound = sound;
  }

  clearSound() {
    if (this.enhancedjournal)
      this.enhancedjournal._backgroundsound[this.document.id] = null;
    else this._backgroundsound = null;
  }
  ...
  _playSound(sound) {
    if (sound.audiofile) {
      ...
      $(
        ".play-journal-sound",
        this.enhancedjournal ? this.enhancedjournal.element : this.trueElement,
      )
        .addClass("loading")
        .find("i")
        .attr("class", "fas fa-sync fa-spin");
      return foundry.audio.AudioHelper.play({
        ...
        if (game.modules.get("monks-sound-enhancements")?.active) {
          game.MonksSoundEnhancements.addSoundEffect(
            soundfile,
            this.document.name,
          );
        }
        $(
          ".play-journal-sound",
          this.enhancedjournal
            ? this.enhancedjournal.element
            : this.trueElement,
        )
          .addClass("active")
          .removeClass("loading")
          .find("i")
          .attr("class", "fas fa-volume-up");
  ...
  static onImageDblClick(event, target) {
    if (this.document.isOwner && !this.document.src)
      this._onEditImage.call(this, event);
    else this._onShowImage.call(this, event);
  }
  ...
  _prepareSubmitData(event, form, formData, updateData) {
    ...
    let relationships = foundry.utils.mergeObject(
      foundry.utils.getProperty(
        submitData,
        "flags.monks-enhanced-journal.relationships",
      ) || {},
    ...
    let items = foundry.utils.mergeObject(
      foundry.utils.getProperty(
        submitData,
    ...
    for (let el of form.elements) {
      ...
      if (field instanceof RadioNodeList) {
        const values = [];
        for (let f of field) {
          if (f.type === "checkbox") values.push(f.checked);
        }
        if (values.length) submitData[el.name] = values;
      }
    }
  ...
  static onSubmit(event, form, formData) {
    ...
    if (this.type == "quest") {
      $(
        `li[data-entry-id="${this.document.id}"]`,
        "#journal,#journal-directory",
      ).attr("status", submitData.flags["monks-enhanced-journal"].status);
    }
    ...
    if (
      ...
      foundry.utils.getProperty(
        submitData,
        "flags.monks-enhanced-journal." + game.user.id,
      )
    ) {
      ...
      MonksEnhancedJournal.emit("saveUserData", {
        ...
        userId: game.user.id,
        userdata: foundry.utils.getProperty(
          submitData,
          `flags.monks-enhanced-journal.${game.user.id}`,
        ),
      });
  ...
  _documentControls() {
    ...
    if (this.document.id)
      ctrls.push({
        id: "locate",
        label: i18n("SIDEBAR.JumpPin"),
        icon: "fas fa-crosshairs",
        visible: game.user.isGM,
        attr: {
          "page-id": this.document.id,
          "journal-id": this.document.parent?.id,
        },
        action: "findMapEntry",
      });
    let defaultSettings = (game.settings.settings.get(
      "monks-enhanced-journal.sheet-settings",
    )?.default || {})[this.constructor.type];
    if (defaultSettings != undefined && Object.keys(defaultSettings).length > 0)
      ctrls.push({
        id: "settings",
        label: i18n("MonksEnhancedJournal.EditFields"),
        icon: "fas fa-cog",
        visible: game.user.isGM,
        action: "editFields",
      });
  ...
  static updateStyle(data, element) {
    if (data == undefined)
      data = foundry.utils.getProperty(
        this.document,
        "flags.monks-enhanced-journal.style",
      );

    if (data == undefined) return;

    let content = $(element).hasClass("editor-parent")
      ? $(element)
      : $(".editor-parent", element || this.trueElement);
  ...
  static onEditDescription(event, target) {
    ...
    if (this.tabGroups["primary"])
      this.changeTab.call(
        this.enhancedjournal || this,
        "description",
        "primary",
        { event, navElement },
      );
    let editing = $(
      ".editor-parent[data-editor-id='description']",
      this.trueElement,
    ).hasClass("editing");
    $(
      ".editor-parent[data-editor-id='description']",
    ...
    $(".nav-button.edit i", this.enhancedjournal?.element || this.element)
      .toggleClass("fa-pencil-alt", editing)
      .toggleClass("fa-save", !editing);
  }

  static onEditNotes(event, target) {
    let editing = $(
      ".editor-parent[data-editor-id='notes']",
      this.trueElement,
    ).hasClass("editing");
    $(".editor-parent[data-editor-id='notes']", this.trueElement).toggleClass(
      "editing",
      !editing,
    );
  }
  ...
  static onEditFields() {
    ...
    new CustomisePage({ document: this.document, journalsheet: this }).render(
      true,
    );
  }
  ...
  async renderPolyglot(html) {
    ...
    $(".editor-display span.polyglot-journal:not(.converted)", html).each(
      function () {
        const lang = this.dataset.language;
        if (!lang) return;

        let text = $(this).html();
        let polyglot = foundry.utils.isNewerVersion(
          game.modules.get("polyglot")?.version,
          "1.7.30",
        )
          ? game.polyglot
          : polyglot?.polyglot;
        ...
        let scramble = polyglot.scrambleString(
          this.textContent,
          that.document.id,
          lang,
        );
        ...
        let languages =
          polyglot.LanguageProvider?.languages ||
          polyglot.languageProvider?.languages;
        ...
        $(this)
          ...
          .attr(
            ...
            game.user.isGM ||
              ...
              polyglot.known_languages.has(lang)
          ...
          .click(function () {
            ...
            if (
              ...
              $(this)
                ...
                .html(data.changed ? data.scramble : data.text)
                .css({ font: data.changed ? data.font : "" });
            }
          });
      },
    );
  ...
  async getItemGroups(purchasing, sort = "name") {
    ...
    for (let [key, item] of Object.entries(items)) {
      ...
      if (!item.system && item.data) item = item.data;
      let requests = Object.entries(
        foundry.utils.getProperty(
          item,
      ...
      let flags =
        foundry.utils.getProperty(item, "flags.monks-enhanced-journal") || {};
      ...
      let cost = price;
      if (flags.cost != undefined) cost = MEJHelpers.getPrice(flags.cost);
      ...
      let itemData = {
        ...
        name: name,
        identifiedname:
          game.user.isGM && identifiedName != name ? identifiedName : null,
        type: item.type,
        img: img,
        hidden: item.hidden,
        lock: item.lock,
        ...
        cost:
          (cost.consume && (game.user.isGM || this.document.isOwner)
            ? "-"
            : "") +
      ...
      if (game.system.id == "dnd5e" && item.system?.rarity) {
        ...
        itemData.rarity = i18n(
          CONFIG.PF2E.rarityTraits[item.system?.traits?.rarity],
        );
      }
      ...
      if (
        ...
        (item.hide !== true &&
          (flags.quantity !== 0 || setting("show-zero-quantity")))
      ) {
    ...
    let currencies = (MonksEnhancedJournal.currencies || []).reduce(
      (a, v) => ({ ...a, [v.id]: v.convert }),
      {},
    );
    ...
    for (let [k, v] of Object.entries(groups)) {
      groups[k].items = groups[k].items.sort((a, b) => {
        ...
        if (sort == "price" || sort == "cost") {
          ...
          aVal =
            (aCurr.value * (currencies[aCurr.currency] || 1)) /
            (currencies[defCurr] || 1);
          bVal =
            (bCurr.value * (currencies[bCurr.currency] || 1)) /
            (currencies[defCurr] || 1);
        }
  ...
  getOfferings() {
    ...
    return (this.document.flags["monks-enhanced-journal"]?.offerings || [])
      .map((o) => {
        if (
          o.hidden &&
          !(
            game.user.isGM ||
            this.document.isOwner ||
            (o.userid == game.user.id && o.state != "cancelled")
          )
        )
          return null;
        ...
        for (let [k, v] of Object.entries(o.currency)) {
          if (v) {
            ...
            items.push({
              ...
              name: `${v} ${i18n(curr?.name) || "Unknown Currency"}`,
            });
          }
        }
        items = items.concat(
  ...
  static async getDocument(data, type, notify = true) {
    ...
    if (data.data) {
      ...
      if (data.lookup) {
        ...
        const entry = pack.index.find(
          (i) => i._id === data.lookup || i.name === data.lookup,
        );
        id = entry.id;
      }
    ...
    if (!document && data.uuid) document = await fromUuid(data.uuid);
  ...
  static async createRequestMessage(entry, item, actor, isShop) {
    ...
    let messageContent = {
      ...
      shop: {
        ...
        img:
          entry.src ||
          `modules/monks-enhanced-journal/assets/${entry.type}.png`,
      },
    };

    //create a chat message
    let whisper = ChatMessage.getWhisperRecipients("GM").map((u) => u.id);
    if (!whisper.find((u) => u == game.user.id)) whisper.push(game.user.id);
    let speaker = ChatMessage.getSpeaker();
    let content = await foundry.applications.handlebars.renderTemplate(
      "./modules/monks-enhanced-journal/templates/request-item.html",
      messageContent,
    );
    let messageData = {
      user: game.user.id,
      ...
      content: content,
      flavor: speaker.alias
        ? format("MonksEnhancedJournal.ActorWantsToPurchase", {
            alias: speaker.alias,
            verb: price
              ? i18n("MonksEnhancedJournal.Purchase").toLowerCase()
              : i18n("MonksEnhancedJournal.Take").toLowerCase(),
          })
        : null,
      whisper: whisper,
      flags: {
  ...
  static async confirmQuantity(item, max, verb, showTotal = true, price) {
    if (!price)
      price = MEJHelpers.getPrice(
        foundry.utils.getProperty(item, "flags.monks-enhanced-journal.cost") ||
          foundry.utils.getProperty(item, "flags.monks-enhanced-journal.price"),
      );

    let maxquantity = max != "" ? parseInt(max) : null;
    if (maxquantity == 1 && !showTotal) return { quantity: 1, price: price };
    ...
    let content = await foundry.applications.handlebars.renderTemplate(
      "/modules/monks-enhanced-journal/templates/confirm-purchase.html",
      data,
    );
    let result = await foundry.applications.api.DialogV2.confirm({
      ...
      render: (evt, dialog) => {
        $('input[name="quantity"]', dialog.element).change((event) => {
          ...
          if (showTotal)
            $(".request-total", dialog.element).html(
              quantity * price.value + " " + price.currency,
            );
        });
      ...
      yes: {
        callback: (event) => {
          ...
          let new_price = game.user.isGM
            ? MEJHelpers.getPrice($('input[name="price"]', form).val())
            : price;
  ...
  static purchaseItem(
    ...
    let items = foundry.utils.duplicate(
    ...
    if (items) {
      ...
      if (item) {
        if (remaining) {
          foundry.utils.setProperty(
            item,
            "flags.monks-enhanced-journal.remaining",
            Math.max(
              foundry.utils.getProperty(
                item,
                "flags.monks-enhanced-journal.remaining",
          ...
          foundry.utils.setProperty(
            item,
            "flags.monks-enhanced-journal.received",
            actor?.name,
          );
          foundry.utils.setProperty(
            item,
            "flags.monks-enhanced-journal.assigned",
            true,
          );
        } else {
          let qty = foundry.utils.getProperty(
            item,
        ...
        } else entry.setFlag("monks-enhanced-journal", "items", items);
        if (chatmessage)
    ...
    if (setting("chat-message")) {
      ...
      if (actor.ownership.default >= CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER)
        ...
        for (let [user, perm] of Object.entries(actor.ownership)) {
          if (
            perm >= CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER &&
            !whisper.find((u) => u == user)
          )
            whisper.push(user);
        }
      }
      let content = await foundry.applications.handlebars.renderTemplate(
        "./modules/monks-enhanced-journal/templates/receive-item.html",
        messageContent,
      );
      let messageData = {
        user: user,
        ...
        style: CONST.CHAT_MESSAGE_STYLES.OTHER,
        content: content,
        flavor: format("MonksEnhancedJournal.ActorPurchasedAnItem", {
          alias: actor.alias ? actor.alias : actor.name,
          verb: purchased
            ? i18n("MonksEnhancedJournal.Purchased").toLowerCase()
            : i18n("MonksEnhancedJournal.Received").toLowerCase(),
        }),
        whisper: whisper,
        flags: {
          "monks-enhanced-journal": messageContent,
  ...
  async getItemData(data) {
    ...
    if (data.pack) result.pack = data.pack;
  ...
  async doClearAllItems(clearLocked = false) {
    if (clearLocked) {
      ...
      let items =
        this.document.getFlag("monks-enhanced-journal", "items") || {};
  ...
  static async editItem(event, target) {
    ...
    if (itemData) {
      ...
      if (itemCls.DEFAULT_OPTIONS) {
        item._sheet = new itemCls({
          document: item,
          alterprice: true,
          addcost: this.document.type == "shop",
        });
      } else {
        item._sheet = new itemCls(item, {
          alterprice: true,
          addcost: this.document.type == "shop",
        });
      }
      ...
      let newSubmit = async (event, form, submitData, updateOptions) => {
        ...
        if (game.system.id == "dnd5e") {
          if (foundry.utils.hasProperty(submitData, "system.properties")) {
            submitData.system.properties = new Set(
              submitData.system.properties,
            );
          }
        }
      ...
      try {
        ...
        ui.notifications.warn(
          i18n("MonksEnhancedJournal.msg.ErrorTryingToEdit"),
        );
      }
    }
  }
  ...
  async rollTable(itemtype = "items", useFrom = false) {
    ...
    let groups = game.tables
      ...
      .reduce((a, v) => ({ ...a, [v.uuid]: v.name }), {});
    rolltables.push({
      text: i18n("MonksEnhancedJournal.RollTables"),
      groups: groups,
    });
    ...
    let lastrolltable =
      that.document.getFlag("monks-enhanced-journal", "lastrolltable") ||
      game.user.getFlag("monks-enhanced-journal", "lastrolltable");

    let table = await fromUuid(lastrolltable);

    let html = await foundry.applications.handlebars.renderTemplate(
      "modules/monks-enhanced-journal/templates/roll-table.html",
      {
        rollTables: rolltables,
        useFrom: useFrom,
        lastrolltable: lastrolltable,
        rollformula: table?.formula,
      },
    );
    foundry.applications.api.DialogV2.confirm({
      window: {
      ...
      yes: {
        callback: async () => {
          ...
          if (table) {
            ...
            let items =
              that.document.getFlag("monks-enhanced-journal", itemtype) || {};
            let newItems = {};
            ...
            let currency =
              that.document.getFlag("monks-enhanced-journal", "currency") || {};
            ...
            for (let i = 0; i < numberof; i++) {
              ...
              if (!table.formula || !available.length) {
                if (table.formula && reset) await table.resetResults();
                else {
                  ui.notifications.warn(
                    ...
                    if (
                      ...
                      for (let tc of textCoins) {
                        if (tc.coin == undefined)
                          ...
                          MonksEnhancedJournal.currencies.find(
                            (c) => c.id == tc.coin,
                          ) == undefined
                        )
                          continue;
                ...
                if (item) {
                  if (itemtype == "items" && item instanceof Item) {
                    ...
                    if (
                      ...
                      let id = itemData._id;
                      itemData =
                        await EnhancedJournalSheet.createScrollFromSpell(
                          itemData,
                        );
                      itemData._id = id;
                    }

                    let oldId = (itemData._id || itemData.id)?.replace(
                      "Item.",
                      "",
                    );
                    let oldItem = Object.values({ ...items, ...newItems }).find(
                      (i) => {
                        let parentId = i.flags[
                          "monks-enhanced-journal"
                    ...
                    if (oldItem && duplicate != "additional") {
                      if (duplicate == "increase") {
                        ...
                        let newqty =
                          parseInt(oldqty) +
                          parseInt(
                            quantity != "" ? await getDiceRoll(quantity) : 1,
                          );
                        foundry.utils.setProperty(
                          oldItem,
                          "flags.monks-enhanced-journal.quantity",
                      ...
                      let sysPrice = MEJHelpers.getSystemPrice(
                        itemData,
                        pricename(),
                      );
                      let price = MEJHelpers.getPrice(sysPrice);
                      ...
                      let sell =
                        adjustment[itemData.type]?.sell ??
                        adjustment?.default?.sell ??
                        1;
                      let cost = MEJHelpers.getPrice(
                        `${price.value * sell} ${price.currency}`,
                      );
                      let itemQuantity =
                        quantity != "" ? await getDiceRoll(quantity) : 1;
                      ...
                      if (useFrom)
                        foundry.utils.setProperty(
                          itemData,
                          "flags.monks-enhanced-journal.from",
                          table.name,
                        );
                    ...
                    if (!newItems[item.id]) {
                      ...
                      if (item.pack) itemData.pack = item.pack;
                      newItems[item.id] = itemData;
                    } else {
                      if (duplicate == "increase") {
                        newItems[item.id].quantity =
                          parseInt(newItems[item.id].quantity) + itemQuantity;
                      }
                    }
                  }
                }
            ...
            if (
              that.document.getFlag("monks-enhanced-journal", "type") ==
                "quest" &&
              itemtype == "items"
            ) {
              let rewardId =
                game.user.getFlag(
                  "monks-enhanced-journal",
                  `reward${this.document.id}`,
                ) || "";
              let rewards = that.document.getFlag(
                "monks-enhanced-journal",
                "rewards",
              );
    ...
    if (container == "relationships" && cascade) {
      ...
      if (journal && journal.pages.size > 0) {
        ...
        if (journal.isOwner && page.isOwner) {
          ...
          MonksEnhancedJournal.emit("deleteRelationship", {
            uuid: journal.uuid,
            id: this.document.id,
            page: this.document.id,
          });
        }
      }
    }
  ...
  static async onRevealRelationship(event, target) {
    ...
    let relationships =
      this.document.getFlag("monks-enhanced-journal", "relationships") || {};
    relationships[li.dataset.id].revealed =
      !relationships[li.dataset.id].revealed;
    await this.document.setFlag(
      "monks-enhanced-journal",
      "relationships",
  ...
  static async onToggleRelationship(event, target) {
    ...
    if (
      journal &&
      (journal instanceof JournalEntryPage || journal.pages.size > 0)
    ) {
      let page =
        journal instanceof JournalEntryPage
          ? journal
          : journal.pages.contents[0];
      let otherRelationships = foundry.utils.duplicate(
        foundry.utils.getProperty(
          page,
          "flags.monks-enhanced-journal.relationships",
        ) || {},
      );
      let otherRelationship = Object.values(otherRelationships).find(
        (value) =>
          value.uuid == this.document.uuid ||
          value.uuid == this.document.parent.uuid,
      );
  ...
  async addItem(data) {
    ...
    let items = foundry.utils.duplicate(
      this.document.flags["monks-enhanced-journal"].items || {},
    );
    let addedItems = [];
    for (let d of data) {
      ...
      if (item) {
        ...
        let existingItem = Object.values(this.getItemList()).find((i) => {
          let parentId = i.flags["monks-enhanced-journal"]?.parentId?.replace(
            "Item.",
            "",
          );
          return !!oldId && !!parentId && parentId == oldId;
        ...
        if (existingItem) {
          ...
          if (
            ...
            (item.type == "spell" && game.system.id == "dnd5e")
          ) {
            let itemData = item.toObject();
            ...
            if (this.document.type == "shop") {
              let adjustment = this.sheetSettings()?.adjustment || {};
              sell =
                adjustment[item.type]?.sell ?? adjustment.default.sell ?? 1;
            }
            ...
            ui.notifications.warn(
              i18n("MonksEnhancedJournal.msg.CannotAddItemType"),
            );
          }
        }
      }
  ...
  static async onLockItem(event, target) {
    ...
    let items = foundry.utils.duplicate(
      this.document.getFlag("monks-enhanced-journal", collection) || {},
    );
  ...
  checkForChanges() {
    return $("prose-mirror", this.trueElement)
      .toArray()
      .some((editor) => {
  ...
  static async _onShowPlayers(event, target, options = {}) {
    event.preventDefault();
    await this.submit();
    return foundry.documents.collections.Journal.showDialog(
      this.document,
      options,
    );
  }
  ...
  static async assignItems(
    items,
    currency = {},
    { clear = false, name = null } = {},
  ) {
    let lootSheet = setting("loot-sheet");
    let lootEntity = setting("loot-entity");
    let collection = EnhancedJournalSheet.isLootActor(lootSheet)
      ? game.actors
      : game.journal;
    ...
    let getLootableName = (entity, source) => {
      ...
      let documents =
        entity == undefined
          ? collection.filter((e) => e.folder == undefined)
          : entity.contents ||
            entity.pages ||
            entity.parent.contents ||
            entity.parent.pages;
      ...
      if (idx > -1) {
        ...
        if (documents && documents.length) {
          for (let doc of documents) {
            if (
              (doc.name.startsWith(start) || start == "") &&
              (doc.name.endsWith(end) || end == "")
            ) {
              let val = Number(
                doc.name.substr(
                  start.length,
                  doc.name.length - start.length - end.length,
                ),
              );
              if (!isNaN(val)) num = Math.max(num || 0, val);
            }
          }
        }
    ...
    if (newitems.length == 0) {
      ...
      if (!Object.values(currency).some((c) => !!c)) return items;
    }
    ...
    if (
      ...
      if (name == undefined || name == "") name = getLootableName(entity, this);

      if (
        (entity instanceof Folder || entity == undefined) &&
        collection.documentName == "JournalEntry"
      ) {
        entity = await JournalEntry.create(
          {
            folder: entity,
            name: name,
      ...
      if (EnhancedJournalSheet.isLootActor(lootSheet)) {
        ...
        entity = await JournalEntryPage.create(
          {
            name: name,
            type: "text",
            flags: {
              "monks-enhanced-journal": { type: "loot", purchasing: "confirm" },
            },
            ownership: { default: CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER },
          },
          { parent: entity, render: false },
        );
        ui.journal.render();
    ...
    if (!entity) {
      ui.notifications.warn(
        i18n("MonksEnhancedJournal.msg.CouldNotFindLootEntity"),
      );
      return items;
    }
    ...
    if (EnhancedJournalSheet.isLootActor(lootSheet)) {
      if (lootSheet == "item-piles") {
        ...
        await ItemPiles.API.addItems(entity, newitems, {
          removeExistingActorItems: clear,
        });
      } else {
      ...
      if (Object.keys(newcurr).length > 0) {
        ...
        else data[currencyname()] = newcurr;
        entity.update({ data: data });
      }
    } else if (lootSheet == "monks-enhanced-journal") {
      let loot = foundry.utils.duplicate(
        entity.getFlag("monks-enhanced-journal", "items") || {},
      );
    ...
    ui.notifications.info(
      format("MonksEnhancedJournal.ItemAddedToActor", { name: entity.name }),
    );
    ...
    for (let item of Object.values(items)) {
      if (
        ...
        item = foundry.utils.mergeObject(item, {
          flags: {
            "monks-enhanced-journal": {
              remaining: 0,
  ...
  static async onItemSummary(event, target) {
    ...
    let itemData = (this.document.getFlag("monks-enhanced-journal", "items") ||
      {})[id];
    if (!itemData) return;
    ...
    if (chatData instanceof Promise) chatData = await chatData;
        ...
        if (game.system.id == "pf2e") {
          ...
          const itemIsPhysical = item.isOfType("physical"),
            ...
            rarityTag = itemIsPhysical
              ? (() => {
                  ...
                  return (
                    ...
                    (span.innerText = game.i18n.localize(
                      CONFIG.PF2E.rarityTraits[item.rarity],
                    )),
                    gmVisibilityWrap(span, item.isIdentified ? "all" : "gm")
                  );
                })()
              : null,
            levelPriceLabel =
              itemIsPhysical && "coins" !== item.system.stackGroup
                ? (() => {
                    const price = item.price.value.toString(),
                      priceLabel = game.i18n.format(
                        "PF2E.Item.Physical.PriceLabel",
                        {
                          price,
                        },
                      ),
                    ...
                    return (
                      (paragraph.dataset.visibility = item.isIdentified
                        ? "all"
                        : "gm"),
                      paragraph.append(
                        levelLabel,
            ...
            properties =
              null !==
                (_b =
                  null === (_a = chatData.properties) || void 0 === _a
                    ? void 0
                    : _a
                        .filter((property) => "string" == typeof property)
                        .map((property) => {
                          const span = document.createElement("span");
                          return (
                    ...
                    .map((trait) => {
                      ...
                      return (
                        ...
                        itemIsPhysical
                          ? gmVisibilityWrap(
                              span,
                              item.isIdentified || !trait.mystified
                                ? "all"
                                : "gm",
                            )
                          : span
          ...
          if (
            ...
            !game.user.isGM &&
            foundry.utils.getProperty(item, "system.unidentified.description")
          )
            chatData = foundry.utils.getProperty(
              item,
              "system.unidentified.description",
            );
          div = $(
            `<div class="item-summary">${typeof chatData == "string" ? chatData : (chatData.description.value ?? chatData.description)}</div>`,
          );
          if (typeof chatData !== "string") {
            ...
            chatData.properties.forEach((p) => {
              if (
                game.system.id == "pf1" &&
                typeof p == "string" &&
                p.startsWith(`${game.i18n.localize("PF1.ChargePlural")}:`)
              ) {
                let prop = p;
                const uses = item.system?.uses;
                if (uses)
                  prop = `${game.i18n.localize("PF1.ChargePlural")}: ${uses.value}/${uses.max}`;
                props.append(`<span class="tag">${prop}</span>`);
              } else props.append(`<span class="tag">${p.name || p}</span>`);
            });
            if (chatData.price != undefined) {
              ...
              props.append(
                `<span class="tag">${i18n("MonksEnhancedJournal.Price")}: ${price}</span>`,
              );
            }
            div.append(props);
          }
  ...
  async addRelationship(relationship, cascade = true) {
    ...
    if (entity.id == this.document.parent.id) return;

    if (!relationship.id) relationship.id = entity.id;
    ...
    if (this.allowedRelationships.includes(type)) {
      let relationships = foundry.utils.duplicate(
        this.document.flags["monks-enhanced-journal"].relationships || {},
      );

      //only add one item
      if (relationships[relationship.id] != undefined) return;
          ...
          sheet.addRelationship(
            {
              ...
              uuid: this.document.parent.uuid,
              hidden: true,
            },
            false,
          );
        } else {
          MonksEnhancedJournal.emit("addRelationship", {
  ...
  static async onOpenRelationship(event, target) {
    ...
    if (!journal.testUserPermission(game.user, "LIMITED"))
      return ui.notifications.error(
        "You don't have permissions to view this document",
      );
  ...
  static clearScale(event) {
    $('button[data-action="resetScale"]', this.trueElement).removeClass(
      "scale",
    );
    $("div.picture-outer", this.trueElement).data({
      size: 100,
      translate: { x: -50, y: -50 },
    });
    $("div.picture-outer .picture-img", this.trueElement).css({
  ...
  scaleImage(event) {
    ...
    if (event.ctrlKey || event.metaKey) {
      ...
      $(".picture-img", event.currentTarget).css({
        transform: `translate(${translate?.x ?? -50}%, ${translate?.y ?? -50}%) scale(${size / 100})`,
      });
  ...
  checkScale(event) {
    if (event.ctrlKey || event.metaKey) {
      $(event.currentTarget)
        .addClass("scaling")
        .data({
          position: $(event.currentTarget).data("translate"),
  ...
  moveScale(event) {
    if ($(event.currentTarget).hasClass("scaling")) {
      ...
      let position = $(event.currentTarget).data("position") || {
        x: -50,
        y: -50,
      };

      let dx =
        ((event.clientX - origin.x) / $(event.currentTarget).width()) * 100;
      let dy =
        ((event.clientY - origin.y) / $(event.currentTarget).height()) * 100;
      ...
      $(".picture-img", event.currentTarget).css({
        transform: `translate(${posX}%, ${posY}%) scale(${size / 100})`,
      });
    }
  }
  ...
  static async splitJournal() {
    ...
    if ($(ctrl).closest(".editor-container,.editor-display").length > 0) {
      ...
      if (selectedHTML.html() != "") {
        let title =
          $("h1,h2,h3,h4", selectedHTML).first().text().trim() ||
          i18n("MonksEnhancedJournal.ExtractedJournalEntry");
        ...
        let types = MonksEnhancedJournal.getDocumentTypes();
        if (types[type]) {
          let newentry = await JournalEntry.create(
            {
              name: title,
              folder: this.document.parent.folder,
              flags: { "monks-enhanced-journal": { type: "journalentry" } },
            },
            { render: false },
          );
          let data = {
            name: title,
            type: "text",
            text: { content: `<p>${selectedHTML.html()}</p>` },
            flags: { "monks-enhanced-journal": { type: "journalentry" } },
          };
          ...
          await this.document.update({
            text: {
              content: $(ctrl)
                .closest(".editor-container,.editor-display")
                .html(),
            },
          });
          if (this.enhancedjournal) this.enhancedjournal.render();
          else this.render();
        }
      } else
  ...
  clickItem(event) {
    ...
    let itemData = (foundry.utils.getProperty(
      this.document,
      "flags.monks-enhanced-journal.items",
    ) || {})[id];
    if (itemData) {
      ...
      if (item.displayCard) item.displayCard();
    }
  }
  ...
  static addLog(options = {}) {
    ...
    if (game.user.isGM) {
      let log = foundry.utils.duplicate(
        this.getFlag("monks-enhanced-journal", "log") || [],
      );
      log.unshift({ actor, item, quantity, price, type, time: Date.now() });
      this.setFlag("monks-enhanced-journal", "log", log);
    } else {
      MonksEnhancedJournal.emit(
        "addLog",
        Object.assign({ entityId: this.uuid }, options),
      );
    }
  }
  ...
  static onConfigureSheet(event) {
    ...
    const docSheetConfigWidth =
      foundry.applications.apps.DocumentSheetConfig.DEFAULT_OPTIONS.position
        .width;
    new foundry.applications.apps.DocumentSheetConfig({
      ...
      position: {
        top: this.position.top + 40,
        left:
          this.position.left + (this.position.width - docSheetConfigWidth) / 2,
      },
    }).render({ force: true });
  }
  ...
  async addActor(data) {
    ...
    if (actor) {
      let update = {};
      if (!this.document.name) update.name = actor.name;
      if (!this.document.src) update.src = actor.img;
  ...
  static onMakeOffering() {
    new MakeOffering({ document: this.document, journalsheet: this }).render(
      true,
    );
  }
  ...
  static onCancelOffer(event, target) {
    ...
    if (game.user.isGM || this.document.isOwner) {
      let offerings = foundry.utils.duplicate(
        this.document.getFlag("monks-enhanced-journal", "offerings"),
      );
      let offering = offerings.find((r) => r.id == id);
      ...
      MonksEnhancedJournal.emit("cancelOffer", {
        id: id,
        uuid: this.document.uuid,
      });
  }
  ...
  static async onAcceptOffer(event, target) {
    ...
    if (!actor) {
      ui.notifications.error(
        "Actor no longer exists, cannot accept this offering",
      );
      return;
    }
    ...
    for (let item of offering.items) {
      ...
      if (item.actorId != offering.actorId) {
        ...
        if (!item.actor) {
          ui.notifications.error(
            `Actor ${item.actorName} no longer exists, cannot accept this offering`,
          );
          return;
        }
      }
      ...
      if (!item.item) {
        ui.notifications.error(
          `Item ${item.itemName} no longer exists, cannot accept this offering`,
        );
        return;
      }
      ...
      if (item.qty > item.max) {
        ui.notifications.error(
          `Not enough of ${item.name} exists, cannot accept this offering`,
        );
        return;
      }
    }
    ...
    let destActor;
    let actorLink = this.document.getFlag("monks-enhanced-journal", "actor");
    if (actorLink) destActor = game.actors.find((a) => a.id == actorLink.id);

    for (let [k, v] of Object.entries(offering.currency)) {
      this.addCurrency(actor, k, -v);
      if (destActor) this.addCurrency(destActor, k, v);
    }
    ...
    for (let item of offering.items) {
      if (destActor) {
        ...
        if (sheet._onDropItem && game.system.id != "sf2e")
          sheet._onDropItem(
            { preventDefault: () => {}, target: { closest: () => {} } },
            itemData,
          );
        else destActor.createEmbeddedDocuments("Item", [itemData]);
      }
  ...
  static async onGenerateName(event, target) {
    ...
    let html = await foundry.applications.handlebars.renderTemplate(
      "modules/monks-enhanced-journal/templates/generate-name.html",
      context,
    );
    ...
    let name = await foundry.applications.api.DialogV2.wait({
      ...
      render: (event, dialog) => {
        ...
        $("button[data-action='generate']", dialog.element).on(
          "click",
          async (event) => {
            event.preventDefault();
            for (let i = 0; i < 5; i++) {
        ...
        $("button[data-action='cancel']", dialog.element).on(
          "click",
          (event) => {
            event.preventDefault();
            dialog.close();
